...
UiController.prototype.roiScreencap = function(roiList, extractChannel){
    ...
    for(var r=0;r<roiList.length;r++){
        ...
        if(x<0) x=0; if(y<0) y=0; if(x+w>imgWidth) w=imgWidth-x; if(y+h>imgHeight) h=imgHeight-y;
        if(w<=0||h<=0) continue;
...
CoopDistortionMonolith.prototype.runFireMonolith = function(){
    ...
    acquireLockCB(this.monolithLock, function(){
        ...
        if(self.monolithFireCount % self.numMonolithGroup === 0){
            ...
            setTimeout(function(){ self.monitorWaveProgressionCB(); }, 0);
        }
...
CoopDistortionMonolith.prototype.updateScreencap = function(cb){
    ...
    var imgs = this.uic.roiScreencap(roiList,"r");
    this.field.updateScreencaps(roiList, imgs);
    if(cb) cb();
};

CoopDistortionMonolith.prototype.monitorWaveProgressionCB = function(){
    var self=this;
    setTimeout(function(){
        self.updateScreencap(function(){
            if(self.field.waveProgressionDetected()){
                acquireLockCB(self.monolithLock, function(){
                    self.copyBarrier();
                    setTimeout(function(){ releaseLock(self.monolithLock); }, self.shortBreakTimeAfterBarrierCopy*1000);
                });
            }
        });
    }, 500);
...
// ===== utils.js =====
function sleep(sec, cb){ setTimeout(cb, sec*1000); }
function periodic(func, intervalSec){
    function loop(){
        var start = Date.now()/1000;
        func();
        var end = Date.now()/1000;
        setTimeout(loop, Math.max(0, intervalSec*1000-(end-start)*1000));
    }
    loop();
}
function acquireLockCB(lock, cb){
    if(lock.value) setTimeout(function(){ acquireLockCB(lock, cb); }, 10);
    else { lock.value=true; cb(); }
}
